...
export function AppointmentCard({
  ...
  const handleCancelAppointment = async () => {
    ...
    const cancelledBeforeStart = now < startTime;
    const refundMsg = cancelledBeforeStart
      ? "Your credits will be refunded."
      : "Credits will not be refunded because the consultation window has already started.";

    if (
      window.confirm(
        `Are you sure you want to cancel this appointment? ${refundMsg}`
      )
    ) {
  ...
  useEffect(() => {
    if (cancelData?.success) {
      toast.success(
        cancelData.refunded
          ? "Appointment cancelled. Credits have been refunded."
          : "Appointment cancelled. No refund — the consultation window had started."
      );
      setOpen(false);
  ...
  // Safety check: if otherParty is missing, show error message
  if (!otherParty) {
    return (
      <Card className="border-red-900/20">
        <CardContent className="p-4">
          <p className="text-red-400 text-sm">
            Error: {otherPartyLabel} information not available for this appointment.
          </p>
        </CardContent>
      </Card>
    );
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-white">
                  {userRole === "DOCTOR"
                    ? otherParty.name || "Unknown Patient"
                    : `Dr. ${otherParty.name || "Unknown Doctor"}`}
                </h3>
                {userRole === "DOCTOR" && (
                  <p className="text-sm text-muted-foreground">
                    {otherParty.email || "No email available"}
                  </p>
                )}
                {userRole === "PATIENT" && (
                  <p className="text-sm text-muted-foreground">
                    {otherParty.specialty || "General Physician"}
                  </p>
                )}
